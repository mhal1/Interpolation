...
def cubicsplineinter(xdata,ydata):
    ...
    #dimensions of matrix n-2 x n-2, n is the number of points
    ...
    # MAKE THIS A TRIDAGANOL MATRIX REMOVE ALL F''_0 TERMS
...
xdata = [-2.1,-1.45,-1.3,-0.2,0.1,0.15,0.8,1.1,1.5,2.8,3.8]

ydata = [0.012155,0.122151,0.184520,0.960789,0.990050,0.977751,0.527292,0.298197,0.105399,3.936690 * 10**(-4),5.355348*10**(-7)]

x1 = cubicsplineinter(xdata,ydata)[0]

y1 = cubicsplineinter(xdata,ydata)[1]

x2 = linearinter(xdata,ydata)[0]

y2 = linearinter(xdata,ydata)[1]

plt.scatter(x1,y1,s=0.2, color = 'C2', label = 'Cubic spline')

plt.scatter(xdata,ydata, color = 'C1', label = 'Data points')

plt.scatter(x2,y2,s=0.2, color = 'C1' , label = 'Linear Interpolation')

plt.legend()

plt.title('Interpolation using cubic splines and linear methods')
